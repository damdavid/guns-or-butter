...
const head = ["turn", ...game.world.nations.map((n) => `nation ${n.id}`)];
...
for (const r of game.rankings()) {
  console.log(`  nation ${r.nation}: ${String(r.population).padStart(5)}  ${String(r.provinces).padStart(3)}  ${r.firepower.toFixed(0).padStart(6)}`);
}
